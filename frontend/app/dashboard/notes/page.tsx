// import Pagination from '@/app/ui/components/pagination';
import Search from '@/app/ui/search';
import NotesTable from '@/app/ui/notes/table';
import { CreateNoteButton } from '@/app/ui/notes/buttons';
import { lusitana } from '@/app/ui/fonts';
import { NotesTableSkeleton } from '@/app/ui/skeletons';
import { Suspense } from 'react';
// import { fetchNotePages } from '@/app/lib/data';

export default async function Page(props: {
  searchParams?: Promise<{
    query?: string;
    page?: string;
  }>;
}) {
  const searchParams = await props.searchParams;
  const query = searchParams?.query || '';
  const currentPage = Number(searchParams?.page) || 1;
  // const totalPages = await fetchNotePages(query);  ページネーションのため、ページ数を返す関数を呼ぶ

  return (
    <div className="w-full">
      <div className="flex w-full items-center justify-between">
        <h1 className={`${lusitana.className} text-2xl`}>Notes</h1>
      </div>
      <div className="mt-4 flex items-center justify-between gap-2 md:mt-8">
        <Search placeholder="Search notes..." />
        <CreateNoteButton />
      </div>
      <Suspense key={query + currentPage} fallback={<NotesTableSkeleton />}>
        <NotesTable query={query} currentPage={currentPage} />
      </Suspense>
      {/*<div className="mt-5 flex w-full justify-center">*/}
      {/*  <Pagination totalPages={totalPages} />*/}
      {/*</div>*/}
    </div>
  );
}
